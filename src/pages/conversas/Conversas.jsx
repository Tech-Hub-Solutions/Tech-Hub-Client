...
const Conversas = () => {
  ...
  const carregarConversas = () => {
    axiosInstance.get(`/conversas`).then((response) => {
      ...
      if (location.state?.usuario) {
        ...
        if (response.data.length > 0) {
          conversa = conversasResponse.find(
            (conversa) => conversa.usuario.id == location.state.usuario.id
          );
        }
        if (conversa) {
      ...
      <div className={styles["conversas"]}>
        ...
        {conversaSelecionada ? (
          ...
          <div className={styles["conversa-default"]}>
            ...
            <p>
              Selecione uma <span>conversa</span> para começar!
            </p>
          </div>
        )}
      </div>
