...
const BannerDescUsuario = (props) => {
  ...
  const favoritar = () => {
    axiosInstance
      .put(`/perfis/favoritar/${usuario?.idUsuario}`)
      .then((res) => {
        props.setUsuario({
          ...usuario,
          isFavorito: !usuario?.isFavorito,
          qtdFavoritos: usuario?.isFavorito
            ? usuario?.qtdFavoritos - 1
            : usuario?.qtdFavoritos + 1,
        });
      })
      .catch((err) => {
  ...
  React.useEffect(() => {
    ...
    const curriculoBaixarButton = (
      <BlueBackgroundButton
        className={styles["botaoCondicional"]}
        valueDisabled={!usuario?.urlCurriculo}
      >
        <a
          href={usuario?.urlCurriculo}
          style={{
            color: usuario?.urlCurriculo
              ? "var(--color-branco)"
              : "var(--color-cinza)",
          }}
          target="_blank"
    ...
    if (!isFreelancer && isOwnProfile) {
      setShowOptions(editarPerfilButton);
  ...
  return (
    <>
      <div
        className={styles["content__banner"]}
        style={{ backgroundImage: `url(${usuario.urlFotoWallpaper || ""})` }}
      >
        {loading.value && loading.tipoArquivo == "WALLPAPER" && (
          <div
            className={styles["content__banner-loading"]}
        ...
        <div className={styles["banner__imagemUsuario"]}>
          ...
          {usuario.isOwnProfile && (
            <div className={styles["banner__alterarImagem"]}>
              <AlterarImagem setLoading={setLoading} loading={loading} />
            </div>
          )}
        </div>
      </div>
      <div className={styles["content__descUsuario"]}>
        <div className={styles["content__infoUsuario"]}>
          <div className={styles["infoUsuario__nome"]}>
            ...
            <h1>{usuario?.nome}</h1>
            {usuario?.pais && <CountryInformation pais={usuario?.pais} />}
            <div className={styles["infoUsuario__icones"]}>
              {usuario.linkLinkedin && (
                <a href={usuario.linkLinkedin} target="_blank">
                  <img src={LinkedinImg} alt="Ícone LinkedIn" />
                </a>
              )}
              {usuario.linkGithub && (
                <a href={usuario.linkGithub} target="_blank">
                  <img src={GitHubImg} alt="Ícone GitHub" />
                </a>
              )}
            </div>
          </div>
          ...
          <div className={styles["precoUsuario"]}>
            {isPerfilFreelancer && (
              <h4>
                {usuario?.precoMedio
                  ? `R$ ${usuario?.precoMedio.toFixed(2)}`
                  : "Preço não informado"}
              </h4>
            )}
          </div>
        </div>
        <div className={styles["descUsuario__button"]}>{showOptions}</div>
      </div>
      <ModalPerfil
        usuario={usuario}
        isModalEdicaoOpen={openModalEdicao}
        setModalEdicaoOpen={setOpenModalEdicao}
        carregarPerfil={props.carregarPerfil}
      />
    </>
  );
};
